...
window.onload = function () {
	...
	function pandaMove () {
		...
		//抬起左边按钮
		$(".arrow_left").on("touchend",function(e){
			e.preventDefault();
		...
		//抬起右按钮
		$(".arrow_right").on("touchend",function(e){
			e.preventDefault();
		...
		getJson("/rank_all",'get',function (data) {
			for (var i = 0; i < data.length; i ++) {
				if (i == 0) {
					...
					var div = $('<div class="items_1"><p class="place_num">'+i+1+'</p><p class="rank_nickname">'+data[i].nickname+'</p><p class="mark_num_1">'+data[i].score+'</p></div>')
				}
